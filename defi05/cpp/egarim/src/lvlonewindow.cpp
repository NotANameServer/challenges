...
void Alive::run() {
    ...
    while (true) {
        QMutex mutex;
        //empêche les autres threads notament l'UI thread d'accèder au valeur m_stop et m_seed.
        mutex.lock();
        if(m_stop) {
            break;
        }
 ...
 * @brief Alive::next
 *
 * Change l'état les valeurs du tableau m_seed en fonction d'un tableau d'état et du résultat de la somme de :
 * m_seed[n-1] + m_seed[n] + m_seed[n+1] avec n ∈ ℕ.
 */
void Alive::next() {
    //Effectue une copie du tableau m_seed pour ne pas changer les états temps que le calcul de tous les éléments n'a pas été effectués.
    ...
    //cas particulier du dernier élment du tableau ou n+1 n'existe pas donc on l'ignore.
