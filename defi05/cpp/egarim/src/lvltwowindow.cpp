...
void Alive2::run() {
    ...
    while (true) {
        QMutex mutex;
        //empêche les autres threads notamment l'UI thread d'accèder au valeur m_stop et m_forests
        mutex.lock();
        if(m_stop) {
            break;
        }
        ...
        //averti l'ihm de se mettre à jour avec la nouvelle valeur du vecteur m_forests
        emit fillGridLayout(0);

        //met le thread en pause une milliseconde
        QThread::msleep(1);
    }
}
 ...
 * @brief Alive2::next
 *
 * Change l'état des valeurs du tableau m_forests en fonction de certaine(s) condition(s) et d'une probabilité.
 */
void Alive2::next() {
    ...
    for (int y=0; y<LvlTwoWindow::FORESTS_HEIGHT; ++y) {
        for (int x=0; x<LvlTwoWindow::FORESTS_WIDTH; ++x) {
            ...
            //on transforme la coordonnée 2D en coordonnée 1D.
            int idx = (y+1) * (x+1) - 1;
            ...
            //on parcourt la liste des conditions par ordre de priorité
            //si une condition est remplie on lance le dé du destin et on stop la boucle quelque soit le résultat.
            foreach(Condition c, m_forests[idx]->getConditions()) {
                ...
                if (nbNeighbor<1) {
                    ...
                    //change l'état de la forêt si proba ok
                    //aléa : Générateurs déterministes
                    if (dice_roll==1) {
    ...
    //met à jour uniqment les forêts qui ont changées d'états.
