 ...
 */
function sortingChallenge(SplFixedArray|array $a, int $n): SplFixedArray|array
{
...
class IntTree
{
    ...
    {
        $node->{$direction} = new IntNode($value, parent: $node);
    }
...
class IntTreeSort
{
     ...
     */
    public function __construct(
        private SplFixedArray|array $list,
        private int $size,
    )
    {
        if ($this->canBeSorted())
     ...
     * @return int[]
     */
    public function getList(): SplFixedArray|array
    {
        return $this->list;
    }
}
...
abstract class AbstractIntTreeSortTest
{
    ...
    private static function getTestMethods(): array
    {
        ...
        {
            if (self::shouldSkipTest($method->getAttributes()))
                continue;
     ...
     * @param int[] $list
     */
    protected static function evaluateSorting(SplFixedArray|array $list): bool
    {
     ...
     * @param int[] $list2
     */
    private static function isListInSameOrder(SplFixedArray|array $list1, SplFixedArray|array $list2): bool
    {
        $size = count($list1);
        if ($size !== count($list2))
            return false;
...
abstract class AbstractTestsCollection extends AbstractIntTreeSortTest
{
    ...
    public static function testRandomArray100000(): bool
    {
    ...
    public static function testRandomArrayMaxInt(): bool
    {
...
abstract class AbstractDumbBenchmark
{
    ...
    public static int|float $benchStart; // CORRECTION added |float
    ...
    {
        $time = hrtime(true) - self::$benchStart;
