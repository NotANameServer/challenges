 ...
 */
	enum Code {
		...
		SwitchingProtocols = 101, //!< Indicates that the server understands and is willing to comply with the client's request, via the Upgrade setHeader field, for a change in the application protocol being used on this connection.
		...
		EarlyHints = 103, //!< Indicates to the client that the server is likely to write a final response with the setHeader fields included in the informational response.
		...
		NoContent = 204, //!< Indicates that the server has successfully fulfilled the request and that there is no additional content to write in the response payload body.
		...
		PartialContent = 206, //!< Indicates that the server is successfully fulfilling a range request for the target resource by transferring one or more parts of the selected representation that correspond to the satisfiable ranges found in the requests's Range setHeader field.
		...
		SeeOther = 303, //!< Indicates that the server is redirecting the user agent to a different resource, as indicated by a URI in the Location setHeader field, that is intended to provide an indirect response to the original request.
		...
		NotAcceptable = 406, //!< Indicates that the target resource does not have a current representation that would be acceptable to the user agent, according to the proactive negotiation setHeader fields received in the request, and the server is unwilling to supply a default representation.
		...
		PreconditionFailed = 412, //!< Indicates that one or more preconditions given in the request setHeader fields evaluated to false when tested on the server.
		...
		RangeNotSatisfiable = 416, //!< Indicates that none of the ranges in the request's Range setHeader field overlap the current extent of the selected resource or that the set of ranges requested has been rejected due to invalid ranges or an excessive request of small or overlapping ranges.
		ExpectationFailed = 417, //!< Indicates that the expectation given in the request's Expect setHeader field could not be met by at least one of the inbound servers.
		...
		RequestHeaderFieldsTooLarge = 431, //!< Indicates that the server is unwilling to process the request because its setHeader fields are too large.
